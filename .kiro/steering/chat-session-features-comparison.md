...
[用户重新打开]
用户：继续刚才的分析
AI：[需要会话管理才能知道"刚才"是什么]
```

**必须实现**：
- 会话存储（JSON 文件或 SQLite）
- 会话恢复功能
- 会话管理 API（列出、删除、恢复、搜索）
- 会话元数据（标题、时间、模型、token 使用量）
